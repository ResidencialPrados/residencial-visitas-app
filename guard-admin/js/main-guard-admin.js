    ...
    .onSnapshot(snapshot => {
      ...
      snapshot.forEach(doc => {
        ...
        if (!v.guardName && v.guardId) {
          db.collection('usuarios').doc(v.guardId).get().then(s => {
            if (s.exists) {
...
// — Crear Usuarios con campos dinámicos (versión “buena”) —
function manejarCreacionUsuarios() {
  ...
  rolSelect.addEventListener('change', () => {
    // muestro/oculto todo el bloque
    camposExtra.style.display = rolSelect.value ? 'block' : 'none';
    msg.textContent = '';
    // oculto todos los campos internos
